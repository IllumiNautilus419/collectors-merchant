...
class WCFMph_Frontend {
  ...
  /**
	...
	function wcfm_chained_product_product_manage_fields( $fields, $product_id, $products_array ) {
		...
		$product_arr = [];
		if( $product_id ) {
			$chained_product_details        = (array) get_post_meta( $product_id, '_chained_product_detail', true );
			$chained_product_ids        = (array) get_post_meta( $product_id, '_chained_product_ids', true );
		...
		$chained_product_fields = array(  
																"_chained_product_detail" => array( 'label' => __( 'Chained Products', 'woocommerce-chained-products' ), 'type' => 'multiinput', 'class' => 'wcfm_ele simple', 'label_class' => 'wcfm_title wcfm_ele simple', 'value' => $chained_product_detail, 'options' =>
																                                   array( "product_id" => array( 'label' => __( 'Product', 'wc-frontend-manager' ) , 'type' => 'select', 'custom_attributes' => array( 'placeholder' => __( 'Search for a product...', 'woocommerce-chained-products' ) ), 'attributes' => array( 'style' => 'width: 60%;' ), 'class' => 'wcfm-select wcfm_ele simple', 'label_class' => 'wcfm_title', 'options' => $product_arr ),
	...
	function wcfm_chained_product_product_manage_fields_variations( $variation_fileds, $variations, $variation_shipping_option_array, $variation_tax_classes_options, $products_array, $product_id, $product_type ) {
		...
		if(!empty($variations)) {
			foreach ($variations as $variation_data) {
				...
				if(!empty($chained_product_detail)){
					foreach ($chained_product_detail as $id => $chained_product_data) {
						if($id != 0) {
							$product_arr[$id]=$chained_product_data['product_name'];
						}
					}
		...
		$chained_product_fields = array(  
																"_chained_product_detail" => array( 'label' => __( 'Chained Products', 'woocommerce-chained-products' ), 'type' => 'multiinput', 'class' => 'wcfm_ele chained_product_detail_variation variable', 'label_class' => 'wcfm_title', 'options' =>
																                                   array( "product_id" => array( 'label' => __( 'Product', 'wc-frontend-manager' ) , 'type' => 'select', 'custom_attributes' => array( 'placeholder' => __( 'Search for a product...', 'woocommerce-chained-products' ) ), 'attributes' => array( 'style' => 'width: 60%;' ), 'class' => 'wcfm-select wcfm_ele variable', 'label_class' => 'wcfm_title', 'options' => $product_arr ),
																                                          ...
																                                          "priced_individually_chained" => array( 'name'=>'priced_individually','label' => __( 'Priced Individually', 'woocommerce-chained-products' ) , 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele variable', 'label_class' => 'wcfm_title checkbox_title wcfm_ele variable', 'value' => 'yes', 'hints' => __( 'Check to manage stock for products listed in chained products, uncheck otherwise.', 'woocommerce-chained-products' ))
																                             ) ),
	...
	function wcfm_chained_product_product_manage_data_variations( $variations, $variation_id, $variation_id_key ) {
		...
		if( $variation_id  ) {
			$chained_product_details        = (array) get_post_meta( $variation_id, '_chained_product_detail', true );
