				...
				if( !$has_error ) {
					...
					if( !$staff_id ) {
						...
						if( !$is_update ) {
							...
							$raw_message = [
								'l10n'	=> [
									'text' 		=> 'A new staff <b>%s</b> added to the store by <b>%s</b>',
									'domain'    => 'wc-frontend-manager-groups-staffs',
									'wrapper'	=> [
										'function' 	=> 'sprintf',
										'args' 		=> [
											$wcfm_staff_manager_form_data['first_name'] . ' ' . $wcfm_staff_manager_form_data['last_name'], 
							...
							$WCFM->wcfm_notification->wcfm_send_direct_message( $author_id, $message_to, $author_is_admin, $author_is_vendor, $wcfm_messages, 'new_staff', true, $raw_message );
						}
