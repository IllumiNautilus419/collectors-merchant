...
class WCFMgs {
	...
	public function redirect_admin_url() {
	...
	function restrict_wcfm_shop_manager_backend() {
	  global $WCFM, $_GET;
		if(is_user_logged_in()) {
			...
			$has_custom_capability = get_user_meta($user_id,'_wcfm_user_has_custom_capability',true);
			$manager_capability = get_user_meta($user_id,'_wcfm_user_capability_options',true);
			$if_assign_group = get_user_meta( $user_id, '_wcfm_vendor_group', true );
			$if_assign_group = !empty($if_assign_group)? array_filter($if_assign_group) : '';
			if(is_admin() && ( 'yes' == $has_custom_capability && empty($if_assign_group)) && !$is_export && !$is_import && in_array( 'shop_manager', (array) $user->roles ) && !defined('DOING_AJAX')) {
				if(!empty($manager_capability) && isset($manager_capability['vnd_wpadmin'])) {
					$this->redirect_admin_url();
				}
			} elseif(!empty($if_assign_group)) {	
				$group_id = reset($if_assign_group);
				$manager_group_capability = get_post_meta($group_id,'_group_capability_options',true);
				if(!empty($manager_group_capability) && isset($manager_group_capability['vnd_wpadmin'])) {
					$this->redirect_admin_url();
				} 
			} elseif ( is_admin() && ( 'yes' == $wcfm_sm_wpadmin ) && !$is_export && !$is_import && in_array( 'shop_manager', (array) $user->roles ) && !defined('DOING_AJAX')) {
				//Fix shop manager backend access capability
				if(!empty($manager_capability) && isset($manager_capability['vnd_wpadmin'])) {
					$this->redirect_admin_url();
				} else {
					$this->redirect_admin_url();
	...
	function restrict_wcfm_shop_staff_backend() {
	  global $WCFM, $_GET;
		if(is_user_logged_in()) {
			...
			$staff_capability = get_user_meta($user_id,'_wcfm_user_capability_options',true);
			$staff_custom_capability = get_user_meta($user_id,'_wcfm_user_has_custom_capability',true);
			
			$is_export = false;
			$is_import = false;
			if( isset($_GET['page']) && ( $_GET['page'] == 'product_exporter' ) ) { $is_export = true; }
			if( isset($_GET['page']) && ( $_GET['page'] == 'product_importer' ) ) { $is_import = true; }
			if($staff_custom_capability == 'yes') {
				if(is_admin() && !empty($staff_capability) && isset($staff_capability['vnd_wpadmin']) && !$is_export && !$is_import && in_array( 'shop_staff', (array) $user->roles ) && !defined('DOING_AJAX')) {	
					$this->redirect_admin_url();
				}				
			} else if( is_admin() && ( 'yes' == $wcfm_ss_wpadmin ) && !$is_export && !$is_import && in_array( 'shop_staff', (array) $user->roles ) && !defined('DOING_AJAX')) {
				//Fix staff backend access capability
					
				if(empty($staff_capability) && isset($staff_capability['vnd_wpadmin'])) {					
					$this->redirect_admin_url();
				} else {
					$this->redirect_admin_url();
				}
			...
			//print_r($wcfm_shop_staff_capability_options);die;
